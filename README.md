...
None |array| Space efficient arrays of uniformly typed numeric values.
None |ast| Abstract Syntax Tree classes and manipulation.
None |asynchat| Deprecated: Support for asynchronous command/response protocols.
None |asyncio| Asynchronous I/O.
None |asyncore| Deprecated: A base class for developing asynchronous socket handling services.
None |atexit| Register and execute cleanup functions.
None |audioop| Deprecated: Manipulate raw audio data.
None |base64 | RFC 4648: Base16, Base32, Base64 Data Encodings; Base85 and Ascii85
None |bdb | Debugger framework.
None |binascii | Tools for converting between binary and various ASCII-encoded binary representations.
None |bisect | Array bisection algorithms for binary searching.
None |builtins | The module that provides the built-in namespace.
None |bz2 | Interfaces for bzip2 compression and decompression.
None | calendar| Functions for working with calendars, including some emulation of the Unix cal program.
None | cgi | Deprecated: Helpers for running Python scripts via the Common Gateway Interface.
None | cgitb | Deprecated: Configurable traceback handler for CGI scripts.
None | chunk | Deprecated: Module to read IFF chunks.
None | cmath | Mathematical functions for complex numbers.
None | cmd | Build line-oriented command interpreters.
None | code | Facilities to implement read-eval-print loops.
None | codecs | Encode and decode data and streams.
None | codeop | Compile (possibly incomplete) Python code.
None | collections | Container datatypes
None | colorsys| Conversion functions between RGB and other color systems.
None | compileall| Tools for byte-compiling all Python source files in a directory tree.
None | concurrent| Execute computations concurrently using threads or processes.
None | configparser| Configuration file parser.
None | contextlib| Utilities for with-statement contexts.
None | contextvars| Context Variables
None | copy| Shallow and deep copy operations.
None | copyreg| Register pickle support functions.
None | cProfile|
None | crypt (Unix)| Deprecated: The crypt() function used to check Unix passwords.
None | csv| Write and read tabular data to and from delimited files.
None | ctypes| A foreign function library for Python.
None | curses (Unix) | An interface to the curses library, providing portable terminal handling.
None | ataclasses | Generate special methods on user-defined classes.
None | datetime | Basic date and time types.
None | dbm | Interfaces to various Unix "database" formats.
None | decimal |Implementation of the General Decimal Arithmetic Specification.
None |difflib| Helpers for computing differences between objects.
None |dis | Disassembler for Python bytecode.
None |distutils|Support for building and installing Python modules into an existing Python installation.
None | doctest | Test pieces of code within docstrings.
None | email | Package supporting the parsing, manipulating, and generating email messages.
None |encodings|
...
None| pickletools| Contains extensive comments about the pickle protocols and pickle-machine opcodes, as well as some useful functions.
...
None|tkinter| Interface to Tcl/Tk for graphical user interfaces
